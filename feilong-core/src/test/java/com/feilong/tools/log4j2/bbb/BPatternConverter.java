package com.feilong.tools.log4j2.bbb;
...
@Plugin(name = "BPatternConverter",category = PatternConverter.CATEGORY)

//ConverterKeys表示的就是自定义的参数,可以多个
@ConverterKeys({ "bbb" })

//定义一个类继承LogEventPatternConverter
public class BPatternConverter extends LogEventPatternConverter{

    private static final BPatternConverter INSTANCE = new BPatternConverter();

    //定义的这个类必须提供一个newInstance方法，
    //参数是final String[] options，
    //返回值为定义的类（对于是否是单例没有明确的要求）
    public static BPatternConverter newInstance(final String[] options){
        return INSTANCE;
    }

    //提供一个私有的构造函数，调用父类的构造函数，函数需要提供两个参数 
    //第一个参数是转换器的名称，第二个是css样式
    private BPatternConverter(){
        super("aaa", "aaa");
    }
