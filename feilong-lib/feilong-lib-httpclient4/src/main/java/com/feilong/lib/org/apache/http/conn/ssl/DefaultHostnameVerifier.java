...
import com.feilong.lib.org.apache.http.conn.util.DomainType;
...
public final class DefaultHostnameVerifier implements HostnameVerifier{
        ...
        if (subjectAlts != null && !subjectAlts.isEmpty()){
            ...
            if (cn == null){
                throw new SSLException(
                                "Certificate subject for <" + host + "> doesn't contain "
    ...
    static boolean matchDomainRoot(final String host,final String domainRoot){
        ...
        return host.endsWith(domainRoot)
                        && (host.length() == domainRoot.length() || host.charAt(host.length() - domainRoot.length() - 1) == '.');
    }
