...
public class CipherUtil{
    ...
    private static byte[] opBytes(byte[] bytes,int opmode,String transformation,Key key){
        ...
        try{
            ...
            return opBytes(bytes, opmode, key, cipher);
     ...
     * @return the byte[]
     */
    public static byte[] opBytes(byte[] bytes,int opmode,String transformation,Key key,Provider provider){
        try{
            Cipher cipher = Cipher.getInstance(transformation, provider);
            return opBytes(bytes, opmode, key, cipher);
        }catch (NoSuchAlgorithmException | NoSuchPaddingException e){
            throw new EncryptionException(Slf4jUtil.format("onewayType:[{}],transformation:[{}]", opmode, transformation), e);
        }
    }
     ...
     * @return the byte[]
     */
    private static byte[] opBytes(byte[] bytes,int opmode,Key key,Cipher cipher){
        ...
        try{
            ...
            throw new EncryptionException(Slf4jUtil.format("opmode:[{}],key:[{}]", opmode, key), e);
        }
    }
}
